  ...
  return (
    ...
    >
      <div className="relative h-44 sm:h-48 bg-slate-100 overflow-hidden shrink-0">
        {rental.cover_image && !imgError ? (
          <img
            src={rental.cover_image}
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-1">
        <h3 className="font-semibold text-slate-800 truncate text-sm">
          ...
          {rental.year && <span className="text-slate-400 font-normal">({rental.year})</span>}
        </h3>
        <p className="text-slate-900 font-bold text-lg sm:text-xl mt-1 tracking-tight">
          {formatPrice(rental.price_per_day)}<span className="text-sm font-normal text-slate-400">/día</span>
        </p>
        <div className="mt-auto pt-3 border-t border-slate-100 flex items-center justify-between gap-1">
          <span className="text-xs text-slate-400 flex items-center gap-1 truncate">
            ...
            <span className="truncate">
              {rental.municipality ? `${rental.municipality}, ` : ''}{rental.province}
            </span>
          </span>
          {rental.seats && (
            <span className="text-xs text-slate-400 font-medium shrink-0">
              {rental.seats} asientos
            </span>
          )}
        </div>
      </div>
